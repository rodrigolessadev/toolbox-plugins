...
DATABASE_ICON_PATH = Path(__file__).resolve().parent / "ui" / "assets" / "database.ico"
...
def set_window_taskbar_icon(icon_path: Optional[Path] = None, hwnd: Optional[int] = None) -> bool:
    """Atualiza o ícone da janela e da barra de tarefas no Windows para o ícone de database."""
    if sys.platform != "win32":
        return False

    target_icon = Path(icon_path) if icon_path else DATABASE_ICON_PATH
