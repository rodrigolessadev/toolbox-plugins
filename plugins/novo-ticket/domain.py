...
TICKET_ICON_PATH = Path(__file__).resolve().parent / "ui" / "assets" / "ticket.ico"


def set_window_taskbar_icon(icon_path: Optional[Path] = None, hwnd: Optional[int] = None) -> bool:
    """Atualiza o ícone da janela e da barra de tarefas no Windows para o ícone de ticket."""
    if sys.platform != "win32":
        return False

    target_icon = Path(icon_path) if icon_path else TICKET_ICON_PATH
