...
def _load_domain():
    spec = importlib.util.spec_from_file_location("log_sanitizer_domain_pkg", Path(__file__).parent / "domain.py")
    mod = importlib.util.module_from_spec(spec)
    spec.loader.exec_module(mod)
    return mod


sanitize_content = _load_domain().sanitize_content
...
def handle_ipc(input_data: dict) -> dict:
    ...
    try:
        res = sanitize_content(raw_content, options)
        out_file = input_data.get("output_file")
        is_dry_run = options.get("dry_run", False)

        if out_file and not is_dry_run:
            Path(out_file).write_text(res["sanitized_content"], encoding="utf-8")
            res["output_file"] = str(out_file)

        # Remove o conteudo integral do payload de resumo de auditoria se output_file foi gravado
        if out_file and not is_dry_run:
            res["sanitized_content"] = f"(Gravado com sucesso em {out_file})"
...
def run_gui():
    ...
    root = tk.Tk()
    root.title("Log Sanitizer — Toolbox")
    ...
    ttk.Label(top, text="⚡ Log Sanitizer", style="Title.TLabel").pack(side="left")
    ...
    def select_file():
        fn = filedialog.askopenfilename(title="Selecione o arquivo de log/json/har")
        if fn:
            file_var.set(fn)
    ...
    def process():
        ...
        if not fn:
            ...
            return
        payload = {"input_file": fn, "options": {"dry_run": True}}
    ...
    ttk.Button(btn_row, text="Sanitizar Log (Dry Run)", style="Accent.TButton", command=process).pack(side="right")
