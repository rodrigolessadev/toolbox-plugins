...
def _load_domain():
    spec = importlib.util.spec_from_file_location("log_timeline_domain_pkg", Path(__file__).parent / "domain.py")
    mod = importlib.util.module_from_spec(spec)
    spec.loader.exec_module(mod)
    return mod


generate_log_timeline = _load_domain().generate_log_timeline
...
def handle_ipc(input_data: dict) -> dict:
    ...
    try:
        ...
        res = generate_log_timeline(combined, options)
        return {
            "protocol_version": "1.0",
            "request_id": req_id,
            "status": "success",
...
def run_gui():
    ...
    root = tk.Tk()
    root.title("Log Timeline — Toolbox")
    ...
    ttk.Label(top, text="⏱️ Linha do Tempo de Incidentes", style="Title.TLabel").pack(side="left")
    ...
    ttk.Button(btn_row, text="Gerar Linha do Tempo", style="Accent.TButton", command=process).pack(side="right")
