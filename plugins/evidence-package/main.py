...
def _load_domain():
    spec = importlib.util.spec_from_file_location("evidence_package_domain_pkg", Path(__file__).parent / "domain.py")
    mod = importlib.util.module_from_spec(spec)
    spec.loader.exec_module(mod)
    return mod


build_evidence_package = _load_domain().build_evidence_package
...
def handle_ipc(input_data: dict) -> dict:
    ...
    if file_path:
        ...
        try:
            content_str = p.read_text(encoding="utf-8-sig", errors="replace")
            parsed_file = json.loads(content_str)
            ...
            return {
                ...
                "error": {
                    "code": "READ_ERROR",
                    "message": f"Erro ao ler arquivo JSON: {e}",
                },
                "warnings": [],
            }
    elif raw_content is not None and isinstance(raw_content, str) and raw_content.strip():
        try:
            parsed_raw = json.loads(raw_content)
            if isinstance(parsed_raw, dict):
                payload.update(parsed_raw)
        except Exception:
            payload["raw_content"] = raw_content

    try:
        res = build_evidence_package(payload, options)
        return {
            "protocol_version": "1.0",
            "request_id": req_id,
            "status": "success",
...
def run_gui():
    ...
    root = tk.Tk()
    root.title("Evidence Package — Toolbox")
    ...
    ttk.Label(top, text="📦 Gerador de Pacote de Evidências", style="Title.TLabel").pack(side="left")

    file_var = tk.StringVar()

    row_file = ttk.Frame(root, style="TFrame")
    row_file.pack(fill="x", padx=16, pady=6)
    ttk.Label(row_file, text="Arquivo de Configuração/Payload:").pack(side="left", padx=(0, 8))
    ttk.Entry(row_file, textvariable=file_var, width=50).pack(side="left", fill="x", expand=True, padx=(0, 8))

    def select_file():
        fn = filedialog.askopenfilename(title="Selecione o arquivo JSON", filetypes=[("JSON files", "*.json"), ("All files", "*.*")])
        if fn:
            file_var.set(fn)
    ...
    def process():
        ...
        if not fn:
            messagebox.showwarning("Aviso", "Selecione um arquivo de configuração JSON.")
            return
    ...
    ttk.Button(btn_row, text="Gerar Pacote de Evidências", style="Accent.TButton", command=process).pack(side="right")
